...
public class ParticleRenderer {
	...
	protected ParticleRenderer(Loader loader, Matrix4f projectionMatrix){
		...
		loader.addInstancedBuffer(quad.getVaoID(), vbo, 1, 4, INSTANCE_DATA_LENGTH, 0);
        loader.addInstancedBuffer(quad.getVaoID(), vbo, 2, 4, INSTANCE_DATA_LENGTH, 4);
        loader.addInstancedBuffer(quad.getVaoID(), vbo, 3, 4, INSTANCE_DATA_LENGTH, 8);
        loader.addInstancedBuffer(quad.getVaoID(), vbo, 4, 4, INSTANCE_DATA_LENGTH, 12);
        loader.addInstancedBuffer(quad.getVaoID(), vbo, 5, 4, INSTANCE_DATA_LENGTH, 16);
        loader.addInstancedBuffer(quad.getVaoID(), vbo, 6, 1, INSTANCE_DATA_LENGTH, 20);
	...
	protected void render(Map<ParticleTexture, List<Particle>> particles, Camera camera){
		...
		prepare();
		for(ParticleTexture pt : particles.keySet()) {
			bindTexture(pt);
			List<Particle> particleList = particles.get(pt);
			pointer = 0;
	...
	private void bindTexture(ParticleTexture pt) {
		if(pt.isAdditive()) {
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, pt.getTextureID());
		shader.loadNumberOfRows(pt.getNumberOfRows());
	}
